...
def main() -> None:
    ...
    publish.add_argument("--output-path", required=True)

    args = parser.parse_args()
    if args.command == "prepare-data":
        _write_json(Path(args.output_path), _run_prepare(args))
    elif args.command == "train-model":
        _write_json(Path(args.output_path), _run_train(args))
    elif args.command == "evaluate-model":
        _write_json(Path(args.output_path), _run_evaluate(args))
    elif args.command == "publish-artifact-metadata":
        _write_json(Path(args.output_path), _run_publish(args))


def _run_prepare(args: argparse.Namespace) -> dict[str, Any]:
    data_config = load_yaml_config(DEFAULT_DATA_CONFIG)
    dataset_id = _clean(args.dataset_id) or config_value(data_config, "dataset", "local-demo")
    return prepare_dataset(
        dataset_id=dataset_id,
        artifact_root=args.artifact_root,
        samples_dir=_clean(args.samples_dir) or config_value(data_config, "samples_dir", "data/samples"),
        project_id=_clean(args.project_id) or config_value(data_config, "project_id"),
        bigquery_dataset=_clean(args.bigquery_dataset) or config_value(data_config, "dataset"),
...
def _run_train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return train_model(
        ...
        model_name=_clean(args.model_name) or config_value(model_config, "model_name", "yolov8n"),
