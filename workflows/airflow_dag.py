...
def _default_params() -> dict[str, Any]:
    ...
    return {
        ...
        "batch_size": int(config_value(model_config, "batch_size", 8)),
        "artifact_root": DEFAULT_ARTIFACT_ROOT,
        "run_label": "manual",
        "min_detections_per_image": 0.1,
        "yolo_dataset_path": None,  # Optional: path to YOLO dataset YAML for real training
    }
...
def _train_task(**context: Any) -> dict[str, Any]:
    ...
    # Optionally run real YOLO training if dataset path is provided
    if cfg.get("yolo_dataset_path"):
        try:
            ...
            trained_model, training_metrics = train_yolo_model(
                config=config,
                dataset_path=cfg["yolo_dataset_path"],
                epochs=int(cfg["epochs"]),
                batch_size=int(cfg.get("batch_size", 8)),
                artifact_dir=run_metadata["artifact_dir"],
            )
            run_metadata["yolo_metrics"] = training_metrics
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"YOLO training in Airflow failed: {e}")
...
def _evaluate_task(**context: Any) -> dict[str, Any]:
    ...
    # Optionally run real YOLO evaluation on sample images
    try:
        from pathlib import Path

        from machledata.data import load_sample_paths
        from machledata.metrics import evaluate_on_images
        from machledata.model import build_model_config

        eval_image_paths = load_sample_paths(cfg.get("samples_dir", "data/samples"))
        if eval_image_paths:
            model_config = load_yaml_config(DEFAULT_MODEL_CONFIG)
            config = build_model_config(
                model_name=cfg["model_name"],
            ...
            yolo_metrics = evaluate_on_images(
                eval_image_paths,
                model_path=training_run.get("model_artifact_path"),
            )
            evaluation_summary["yolo_metrics"] = yolo_metrics
    except Exception as e:
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"YOLO evaluation in Airflow failed: {e}")
