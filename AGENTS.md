...
Write Python 3.11+ using PEP 8 with 4-space indentation, `snake_case` for functions and variables, `PascalCase` for classes, and lowercase module names. Keep modules focused and name scripts descriptively, for example `train.py` or `predict.py`.

Prefer type hints and concise docstrings. Keep docstrings short — one line describing the purpose is enough for most functions.
